...
def CalculeConESN(pDataTrain, pDataTest, pCantidadVarAPredecir, pNeuronas, 
                  a, gNumIteraciones, tMetodo, tIpatron, pTipoProceso):    
    global gInitLenPorcentaje
    ...
    pInitLen = int(round(pTestLen*gInitLenPorcentaje))   # 10  Este es un valor recomendado
    ######################################################
    ...
    for t in range(pTestLen):
        ...
        u = u.T
        x = (1 - a) * x 
        x += a * np.tanh(np.dot(Win, np.vstack(([[1]], u))) + np.dot(W, x))
...
def newMethod_2_entrene(pTrainingSet, pRespuestasTrainSet):
    ...
    for tFil in range(tFilas):
        ...
        for tCol in range(tColumnas):
            tEfectoTemp[tCol] =    tTemp[tCol] / tTotalTemp
            if tResult[7][tCol] != 0:
                tTotalAporteMax += ( abs(tResult[7][tCol]-pTrainingSet[tFil][tCol]) / tResult[7][tCol] )
        tTodosLosEfectos.append(tEfectoTemp)
        tResult[3] = tResult[3] + tTemp
        tResult[4] = tResult[4] + tEfectoTemp
...
def EjecuteSVR_ESN(tDirectorio, tLugar, tArchivoEntrada, tArchivoConfig, pTipoProceso,
    ...
    tAlgoritmos = [ tAlgoritmo1, tAlgoritmo2, tAlgoritmo3, tAlgoritmo4, tAlgoritmo5,
                    tAlgoritmo6, tAlgoritmo7,  tAlgoritmo10,
                    tAlgoritmo13, tAlgoritmo14, tAlgoritmo15 ]
    
    tAlgoritmos = [ tAlgoritmo5 ]  
    ...
    if gDiscreticeEspecial:  # Discretiza Atributos independiente de lo que diga el archivo de configuración
        tMediasAtrib = tMatrizDatos.mean()
        tSTDsAtrib = tMatrizDatos.std()
        tCVAtrib= tSTDsAtrib/ tMediasAtrib
        tCVAtrib=  np.ceil (tCVAtrib * 100)      
    tDecisionDiscretizar = {}
    for tNombreCol, tDatosCol in tMatrizDatos.iteritems():
        if gDiscreticeEspecial:
            tCuantosRangos = int(tCVAtrib.loc[tNombreCol])
        else:
        ...
        if (tDecisionDiscretizar[tNombreCol] == "s") or (gDiscreticeEspecial 
                and (tNombreCol != tNombreVariableAPredecir)  ):   
            tLista = []
                ...
                for tVez in range(gNumIteraciones):
                    ...
                    for tAlgoritmo in tAlgoritmos:   
                        ...
                        ###############################################################################        
                            ...
                            tModelo = linear_model.LinearRegression(fit_intercept=False, normalize=False, copy_X=True) 
                            tModelo.fit(tTrainingSet, tRespuestasTrainSet)
                            tResultado = tModelo.predict(tCrossValidationSet)
                            tDetalleAlg = str(tModelo.get_params(deep=True))+" - "+str(tModelo.coef_)
                        ...
                        ###############################################################################        
                            ...
                            tModelo = linear_model.ElasticNet(alpha=0.5, fit_intercept=False, normalize=False, copy_X=True)
                            tModelo.fit(tTrainingSet, tRespuestasTrainSet)
                            tResultado = tModelo.predict(tCrossValidationSet)
                            tDetalleAlg = str(tModelo.get_params(deep=True))+" - "+str(tModelo.coef_)
                        ...
                        ###############################################################################        
                            ...
                            tModelo = Ridge(alpha=0.1, fit_intercept=False, normalize=False, copy_X=True)
                            tModelo.fit(tTrainingSet, tRespuestasTrainSet)
                            tResultado = tModelo.predict(tCrossValidationSet)
                            tDetalleAlg = str(tModelo.get_params(deep=True))+" - "+str(tModelo.coef_)
                        ...
                        ###############################################################################        
                            ...
                            tAjusteSVR_rbf = SVR(kernel='rbf', C=tSVR_C, gamma=tSVR_gamma,degree=t_degree)    
                        ...
                        ###############################################################################
                            ...
                            tAjusteSVR_lin = SVR(kernel='linear', C=tSVR_C, degree=t_degree, tol = 1e-3 , epsilon = 0.1, shrinking = True)
                            #tAjusteSVR_lin = SVR(kernel='linear', C=0.00000001, degree=3, epsilon=0.0)
                            #tAjusteSVR_lin = SVR(kernel='linear', C=100, max_iter=-1, epsilon=0.01, tol=1e-5 )
                        ...
                        ###############################################################################
                            ...
                            tAjusteSVR_sig = SVR(kernel='sigmoid', C=tSVR_C, gamma=tSVR_gamma, degree=t_degree) 
                        ...
                        ###############################################################################
                            ...
                            tResultado = newMethod_2_prediga(tEntrenadoNewMethod, tCrossValidationSet,pLimiteNM)                         
                            tDetalleAlg = "Limite: "  + str(pLimiteNM)                             
                        ...
                        ###############################################################################
                            ...
                            tBitacora.write("ESN"+ "\n")
                            pNeuronas = int( round( len(pTrainESN) * 0.4 ))
                            tResultado = CalculeConESN(pTrainESN, pCrossESN, gCantidadVarAPredecir,
                            pNeuronas, pLeakingRate, gNumIteraciones, 
                                                    tMetodo, tIpatron, pTipoProceso)                    
                            tDetalleAlg =  "Neuronas= "+str(pNeuronas)+' - Leaking rate='+str(pLeakingRate)
                        else:
...
if __name__ == "__main__":
    ...
    ######################################################################################
    tLugarValidacion = "La Rita"
    tAlgoritmosArch = "EnPat=T TasaC=F Discretice=FyT"
    gArchResultValidacion =   "2016-06-23-Sigatoka - "+tLugarValidacion+tAlgoritmosArch+ tAgregueAlNombre+" - "+str(gCuantosPeriodosValidar)
    gArchDelProyectoInicial = "2016-06-23-Sigatoka - "+tLugarValidacion+tAlgoritmosArch + tAgregueAlNombre
    gNombreArchivoConPatronDefinido = "2016-06-23-Sigatoka - "+tLugarValidacion+tAlgoritmosArch + tAgregueAlNombre
    ######################################################################################
    
    
    # tipos de proceso   "Pruebas"    "Siguiente"
    gTipoProceso = "Siguiente"
    ...
    gListaDiscreticeEspecial = [ False]
         
    gListagEscaleDatosEntreAyB = [False]
